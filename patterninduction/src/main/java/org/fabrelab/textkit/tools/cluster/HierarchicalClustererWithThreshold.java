...
*/
public class HierarchicalClustererWithThreshold extends AbstractClusterer implements OptionHandler, CapabilitiesHandler, Drawable {
  ...
  int maxLinkDistance = 2;
  
  public int getMaxLinkDistance() {
	return maxLinkDistance;
  }
  
  public void setMaxLinkDistance(int maxLinkDistance) {
...
/** distance function used for comparing members of a cluster **/
  ...
  public void buildClusterer(Instances data) throws Exception {
    ...
    // & collect hierarchy
    int iCurrent = 0;
    m_clusters = new Node[numClusters];
  ...
  void doLinkClustering(int nClusters, Vector<Integer>[] nClusterID, Node [] clusterNodes) {
    ...
    while (nClusters > 1) {
      ...
      if (m_bDebug) {
        ...
        if(fMinDistance>maxLinkDistance){
  ...
  public int numberOfClusters() throws Exception {
    return Math.min(numClusters, m_instances.numInstances());
  }
  ...
  public static void main(String [] argv) {
    runClusterer(new HierarchicalClustererWithThreshold(), argv);
  }
